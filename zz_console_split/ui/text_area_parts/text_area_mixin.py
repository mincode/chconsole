...
from zz_console_split.ui.signal_content import SignalContent
...
class _ViewportFilter(BaseEventFilter):
    ...
    def eventFilter(self, obj, event):
        ...
        if event_type == QtCore.QEvent.MouseMove:
            intercepted = False
            anchor = self.area.anchorAt(event.pos())
...
class _TextAreaFilter(BaseEventFilter):
    ...
    def eventFilter(self, obj, event):
        ...
        if event_type == QtCore.QEvent.KeyPress:
            ...
            key = event.key()
            if control_key_down(event.modifiers()) and \
        ...
        # Override shortcuts for all filtered widgets.
                ...
                control_key_down(event.modifiers()) and \
...
class TextAreaMixin(object):
    ...
    def __init__(self, font_family, gui_completion):
        self.text_area_filter = _TextAreaFilter(self)
        self.installEventFilter(self.text_area_filter)
        self.viewport_filter = _ViewportFilter(self)
        self.installEventFilter(self.viewport_filter)
