...
class DefaultNames:
    ...
    kernel_user = 'ch_kernel' # default kernel user name
    gate = 'in.chgate.net'  # ssh server that can see host
    gate_user = 'chconnect'  # ssh user name
    gate_pem = os.path.normcase('/home/ec2-user/chconnect.pem')  # private ssh key for ssh user
    local_conn_dir = os.path.normpath('/var/www/html')  # path to the local location of the connection file
    remote_conn_dir = '~'  # path to the remote location of the connection file
    data_key = 'ch_tunnel'  # key for storing data in addition to connection file keys
